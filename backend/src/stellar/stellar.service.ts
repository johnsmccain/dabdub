...
export class StellarService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    this.issuerPublicKey =
      this.configService.get<string>('STELLAR_ISSUER_PUBLIC_KEY') || '';

    this.logger.log(
      `Connected to Stellar Horizon at ${horizonUrl} with network passphrase ${this.networkPassphrase}`,
    );
  }
  ...
  async createAccount(): Promise<{ publicKey: string; secret: string }> {
    ...
    if (this.networkPassphrase === StellarSdk.Networks.TESTNET) {
      try {
        ...
        this.logger.error(
          `Failed to fund account with friendbot: ${e.message}`,
        );
      }
    }
    return {
  ...
  monitorTransactions(accountId: string, callback: (payment: any) => void) {
    ...
    this.streamClose = this.server
      ...
      .stream({
        ...
        onerror: (error) => {
          this.logger.error(
            `Error in payment stream: ${JSON.stringify(error)}`,
          );
        },
      });
  }
